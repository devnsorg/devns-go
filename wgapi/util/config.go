...
//https://raw.githubusercontent.com/nmiculinic/wg-quick-go/master/config.go
...
var _ encoding.TextMarshaler = (*WgQuickConfig)(nil)
var _ encoding.TextUnmarshaler = (*WgQuickConfig)(nil)
...
func (cfg *WgQuickConfig) UnmarshalText(text []byte) error {
	*cfg = WgQuickConfig{} // Zero out the config
	state := unknown
...
func parseInterfaceLine(cfg *WgQuickConfig, lhs string, rhs string) error {
	switch lhs {
	case "Address":
